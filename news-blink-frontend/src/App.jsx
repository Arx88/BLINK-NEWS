...
const fetchNews = async (tab: string = 'ultimas'): Promise<NewsItem[]> => {
  try {
...
const voteOnArticle = async (articleId: string, voteType: 'like' | 'dislike'): Promise<void> => {
  try {
...
const VoteSystem = ({ 
  ...
  const handleVote = async (voteType: 'like' | 'dislike') => {
    if (userVote === voteType) return; // Evitar votos duplicados
...
const NewsCard = ({ 
  ...
  votes,
  onCardClick 
}: NewsItem & { isHot?: boolean; onCardClick: (id: string) => void }) => (
...
const Home = () => {
  ...
  const loadNews = async (tab: string) => {
    setLoading(true);
    try {
