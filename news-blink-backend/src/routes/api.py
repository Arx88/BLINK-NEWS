...
def vote_blink(id):
    ...
    app_logger.info(f"vote_blink called for id: {id}. Payload: {data}")
    try:
        ...
        app_logger.info(f"Current votes for {id}: +{current_positive_votes}/-{current_negative_votes}")
        ...
        app_logger.info(f"New votes for {id}: +{blink_data['positive_votes']}/-{blink_data['negative_votes']}")

        save_blink_data(id, blink_data)
        app_logger.info(f"Saved blink_data for {id}.")

        blink_data['interest'] = calculate_interest(blink_data['positive_votes'], blink_data['negative_votes'])
        app_logger.info(f"Recalculated interest for {id}: {blink_data['interest']}%")
        ...
        app_logger.info(f"Returning updated blink data for {id} (summary): {final_blink_summary}")
